...
В репозиторий интегрирован CI/CD пайплайн (`.github/workflows/validate.yml`), который автоматически запускается при каждом обновлении кода:
* **Проверка синтаксиса:** запуск `docker compose config`.
* **Тест развертывания:** имитация запуска всей инфраструктуры в облаке GitHub.
* **Healthcheck:** автоматическая проверка доступности WordPress через `curl` после старта.
Это гарантирует, что проект всегда остается в рабочем состоянии.
